...
public class BufferedTcpServer {
    ...
    private static final LinkedBlockingQueue<NewClientMessage> writeQueue = new LinkedBlockingQueue<>();
    private static final ByteBuffer sendBuffer = ByteBuffer.allocate(128 * 1024);
    private static final NewClientMessageEncoder encoder = new NewClientMessageEncoder();
    ...
    private static void createTcpServer0(int port) throws IOException {
        ...
        NewClientMessageDecoder clientMessageDecoder = new NewClientMessageDecoder(null, new Consumer<NewClientMessage>() {
            @Override
            public void accept(NewClientMessage clientMessage) {
                writeQueue.offer(clientMessage);
            }
        });
